...
fn renderRoot(
    ...
    while (true) {
        ...
        try renderTopLevelDecl(allocator, stream, tree, &start_col, decl);
...
fn renderTopLevelDecl(allocator: *mem.Allocator, stream: var, tree: *ast.Tree, start_col: *usize, decl: *ast.Node) (@TypeOf(stream).Child.Error || Error)!void {
    try renderContainerDecl(allocator, stream, tree, start_col, decl, .Newline);
}

fn renderContainerDecl(allocator: *mem.Allocator, stream: var, tree: *ast.Tree, start_col: *usize, decl: *ast.Node, space: Space) (@TypeOf(stream).Child.Error || Error)!void {
    switch (decl.id) {
        .FnProto => {
            const fn_proto = @fieldParentPtr(ast.Node.FnProto, "base", decl);

            try renderDocComments(tree, stream, fn_proto, start_col);

            if (fn_proto.body_node) |body_node| {
                try renderExpression(allocator, stream, tree, start_col, decl, .Space);
                try renderExpression(allocator, stream, tree, start_col, body_node, space);
            } else {
                try renderExpression(allocator, stream, tree, start_col, decl, .None);
                try renderToken(tree, stream, tree.nextToken(decl.lastToken()), start_col, space);
            }
        },
        ...
        .Use => {
            ...
            if (use_decl.visib_token) |visib_token| {
                try renderToken(tree, stream, visib_token, start_col, .Space); // pub
            }
            try renderToken(tree, stream, use_decl.use_token, start_col, .Space); // usingnamespace
            try renderExpression(allocator, stream, tree, start_col, use_decl.expr, .None);
            try renderToken(tree, stream, use_decl.semicolon_token, start_col, space); // ;
        },

        .VarDecl => {
            const var_decl = @fieldParentPtr(ast.Node.VarDecl, "base", decl);

            try renderDocComments(tree, stream, var_decl, start_col);
            try renderVarDecl(allocator, stream, tree, start_col, var_decl);
        },

        .TestDecl => {
            const test_decl = @fieldParentPtr(ast.Node.TestDecl, "base", decl);

            try renderDocComments(tree, stream, test_decl, start_col);
            try renderToken(tree, stream, test_decl.test_token, start_col, .Space);
            try renderExpression(allocator, stream, tree, start_col, test_decl.name, .Space);
            try renderExpression(allocator, stream, tree, start_col, test_decl.body_node, space);
        },

        .ContainerField => {
            const field = @fieldParentPtr(ast.Node.ContainerField, "base", decl);

            try renderDocComments(tree, stream, field, start_col);
            if (field.comptime_token) |t| {
                try renderToken(tree, stream, t, start_col, .Space); // comptime
            }
            ...
            if (field.type_expr == null and field.value_expr == null) {
                try renderToken(tree, stream, field.name_token, start_col, last_token_space); // name
            } else if (field.type_expr != null and field.value_expr == null) {
                try renderToken(tree, stream, field.name_token, start_col, .None); // name
                try renderToken(tree, stream, tree.nextToken(field.name_token), start_col, .Space); // :

                if (field.align_expr) |align_value_expr| {
                    try renderExpression(allocator, stream, tree, start_col, field.type_expr.?, .Space); // type
                    ...
                    try renderToken(tree, stream, align_kw, start_col, .None); // align
                    try renderToken(tree, stream, lparen_token, start_col, .None); // (
                    try renderExpression(allocator, stream, tree, start_col, align_value_expr, .None); // alignment
                    try renderToken(tree, stream, rparen_token, start_col, last_token_space); // )
                } else {
                    try renderExpression(allocator, stream, tree, start_col, field.type_expr.?, last_token_space); // type
                }
            } else if (field.type_expr == null and field.value_expr != null) {
                try renderToken(tree, stream, field.name_token, start_col, .Space); // name
                try renderToken(tree, stream, tree.nextToken(field.name_token), start_col, .Space); // =
                try renderExpression(allocator, stream, tree, start_col, field.value_expr.?, last_token_space); // value
            } else {
                try renderToken(tree, stream, field.name_token, start_col, .None); // name
                try renderToken(tree, stream, tree.nextToken(field.name_token), start_col, .Space); // :
                ...
                if (field.align_expr) |align_value_expr| {
                    ...
                    try renderToken(tree, stream, lparen_token, start_col, .None); // (
                    try renderExpression(allocator, stream, tree, start_col, align_value_expr, .None); // alignment
                    try renderToken(tree, stream, rparen_token, start_col, .Space); // )
                } else {
                    try renderExpression(allocator, stream, tree, start_col, field.type_expr.?, .Space); // type
                }
                try renderToken(tree, stream, tree.prevToken(field.value_expr.?.firstToken()), start_col, .Space); // =
                try renderExpression(allocator, stream, tree, start_col, field.value_expr.?, last_token_space); // value
            }

            if (src_has_trailing_comma) {
                const comma = tree.nextToken(field.lastToken());
                try renderToken(tree, stream, comma, start_col, space);
            }
        },
        ...
        .DocComment => {
            ...
            while (it.next()) |line_token_index| {
                try renderToken(tree, stream, line_token_index.*, start_col, .Newline);
            }
        },
        else => unreachable,
    }
...
fn renderExpression(
    ...
    switch (base.id) {
        .Identifier => {
            ...
            return renderToken(tree, stream, identifier.token, start_col, space);
        },
        .Block => {
            const block = @fieldParentPtr(ast.Node.Block, "base", base);

            if (block.label) |label| {
                try renderToken(tree, stream, label, start_col, Space.None);
                try renderToken(tree, stream, tree.nextToken(label), start_col, Space.Space);
            }

            if (block.statements.len == 0) {
                stream.pushIndent();
                defer stream.popIndent();
                try renderToken(tree, stream, block.lbrace, start_col, Space.None);
            } else {
                stream.pushIndent();
                defer stream.popIndent();
                try renderToken(tree, stream, block.lbrace, start_col, Space.Newline);

                var it = block.statements.iterator(0);
                while (it.next()) |statement| {
                    try renderStatement(allocator, stream, tree, start_col, statement.*);
            ...
            return renderToken(tree, stream, block.rbrace, start_col, space);
        },
        .Defer => {
            const defer_node = @fieldParentPtr(ast.Node.Defer, "base", base);

            try renderToken(tree, stream, defer_node.defer_token, start_col, Space.Space);
            return renderExpression(allocator, stream, tree, start_col, defer_node.expr, space);
        },
        .Comptime => {
            const comptime_node = @fieldParentPtr(ast.Node.Comptime, "base", base);

            try renderToken(tree, stream, comptime_node.comptime_token, start_col, Space.Space);
            return renderExpression(allocator, stream, tree, start_col, comptime_node.expr, space);
        },
        ...
        .Suspend => {
            ...
            if (suspend_node.body) |body| {
                try renderToken(tree, stream, suspend_node.suspend_token, start_col, Space.Space);
                return renderExpression(allocator, stream, tree, start_col, body, space);
            } else {
                return renderToken(tree, stream, suspend_node.suspend_token, start_col, space);
            }
        },
        ...
        .InfixOp => {
            ...
            try renderExpression(allocator, stream, tree, start_col, infix_op_node.lhs, op_space);
            ...
            try renderToken(tree, stream, infix_op_node.op_token, start_col, after_op_space);
            const new_indent:u8 = if (after_op_space == Space.Newline) indent_delta else 0;

            stream.pushIndentN(new_indent);
            defer stream.popIndent();

            switch (infix_op_node.op) {
                ast.Node.InfixOp.Op.Catch => |maybe_payload| if (maybe_payload) |payload| {
                    try renderExpression(allocator, stream, tree, start_col, payload, Space.Space);
                },
                else => {},
            }

            return renderExpression(allocator, stream, tree, start_col, infix_op_node.rhs, space);
        },
            ...
            switch (prefix_op_node.op) {
                .PtrType => |ptr_info| {
                    ...
                    if (ptr_info.sentinel) |sentinel| {
                        ...
                        try renderToken(tree, stream, colon_token, start_col, Space.None); // :
                        const sentinel_space = switch (op_tok_id) {
                            .LBracket => Space.None,
                            else => Space.Space,
                        };
                        try renderExpression(allocator, stream, tree, start_col, sentinel, sentinel_space);
                    }
                    switch (op_tok_id) {
                    ...
                    if (ptr_info.allowzero_token) |allowzero_token| {
                        try renderToken(tree, stream, allowzero_token, start_col, Space.Space); // allowzero
                    }
                    if (ptr_info.align_info) |align_info| {
                        ...
                        try renderToken(tree, stream, align_token, start_col, Space.None); // align
                        try renderToken(tree, stream, lparen_token, start_col, Space.None); // (

                        try renderExpression(allocator, stream, tree, start_col, align_info.node, Space.None);
                        ...
                        if (align_info.bit_range) |bit_range| {
                            ...
                            try renderToken(tree, stream, colon1, start_col, Space.None); // :
                            try renderExpression(allocator, stream, tree, start_col, bit_range.start, Space.None);
                            try renderToken(tree, stream, colon2, start_col, Space.None); // :
                            try renderExpression(allocator, stream, tree, start_col, bit_range.end, Space.None);

                            const rparen_token = tree.nextToken(bit_range.end.lastToken());
                            try renderToken(tree, stream, rparen_token, start_col, Space.Space); // )
                        } else {
                    ...
                    if (ptr_info.const_token) |const_token| {
                        try renderToken(tree, stream, const_token, start_col, Space.Space); // const
                    }
                    if (ptr_info.volatile_token) |volatile_token| {
                        try renderToken(tree, stream, volatile_token, start_col, Space.Space); // volatile
                    }
                },

                .SliceType => |ptr_info| {
                    try renderToken(tree, stream, prefix_op_node.op_token, start_col, Space.None); // [
                    if (ptr_info.sentinel) |sentinel| {
                        const colon_token = tree.prevToken(sentinel.firstToken());
                        try renderToken(tree, stream, colon_token, start_col, Space.None); // :
                        try renderExpression(allocator, stream, tree, start_col, sentinel, Space.None);
                        try renderToken(tree, stream, tree.nextToken(sentinel.lastToken()), start_col, Space.None); // ]
                    } else {
                        try renderToken(tree, stream, tree.nextToken(prefix_op_node.op_token), start_col, Space.None); // ]
                    }
                    ...
                    if (ptr_info.const_token) |const_token| {
                        try renderToken(tree, stream, const_token, start_col, Space.Space);
                    }
                    if (ptr_info.volatile_token) |volatile_token| {
                        try renderToken(tree, stream, volatile_token, start_col, Space.Space);
                    }
                },
                ...
                .ArrayType => |array_info| {
                    ...
                    try renderToken(tree, stream, lbracket, start_col, Space.None); // [
                    ...
                    const new_indent:u8 = if (ends_with_comment) indent_delta else 0;
                    const new_space = if (ends_with_comment) Space.Newline else Space.None;
                    {
                        stream.pushIndentN(new_indent);
                        defer stream.popIndent();
                        try renderExpression(allocator, stream, tree, start_col, array_info.len_expr, new_space);
                    }
                    ...
                    try renderToken(tree, stream, rbracket, start_col, Space.None); // ]
                },
                .BitNot,
                .BoolNot,
                .Negation,
                ...
                .Resume,
                => {
                    try renderToken(tree, stream, prefix_op_node.op_token, start_col, Space.Space);
                },

                .Await => |await_info| {
                    if (await_info.noasync_token) |tok| {
                        try renderToken(tree, stream, tok, start_col, Space.Space);
                    }
                    try renderToken(tree, stream, prefix_op_node.op_token, start_col, Space.Space);
                },
            }

            return renderExpression(allocator, stream, tree, start_col, prefix_op_node.rhs, space);
        },
            ...
            switch (suffix_op.op) {
                .Call => |*call_info| {
                    if (call_info.async_token) |async_token| {
                        try renderToken(tree, stream, async_token, start_col, Space.Space);
                    }

                    try renderExpression(allocator, stream, tree, start_col, suffix_op.lhs.node, Space.None);

                    const lparen = tree.nextToken(suffix_op.lhs.node.lastToken());

                    if (call_info.params.len == 0) {
                        try renderToken(tree, stream, lparen, start_col, Space.None);
                        return renderToken(tree, stream, suffix_op.rtoken, start_col, space);
                    }
                    ...
                    if (src_has_trailing_comma) {
                        try renderToken(tree, stream, lparen, start_col, Space.Newline);
                        ...
                        while (it.next()) |param_node| {
                            ...
                            if (it.peek()) |next_node| {
                                try renderExpression(allocator, stream, tree, start_col, param_node.*, Space.None);
                                const comma = tree.nextToken(param_node.*.lastToken());
                                try renderToken(tree, stream, comma, start_col, Space.Newline); // ,
                                try renderExtraNewline(tree, stream, start_col, next_node.*);
                            } else {
                                try renderExpression(allocator, stream, tree, start_col, param_node.*, Space.Comma);
                            }
                        }
                        return renderToken(tree, stream, suffix_op.rtoken, start_col, space);
                    }
                    ...
                    while (it.next()) |param_node| {
                        ...
                        if (it.peek() != null) {
                            const comma = tree.nextToken(param_node.*.lastToken());
                            try renderToken(tree, stream, comma, start_col, Space.Space);
                        }
                    }
                    return renderToken(tree, stream, suffix_op.rtoken, start_col, space);
                },
                ...
                .ArrayAccess => |index_expr| {
                    ...
                    const ends_with_comment = tree.tokens.at(rbracket - 1).id == .LineComment;
                    {
                        ...
                        stream.pushIndentN(new_indent);
                        defer stream.popIndent();
                        try renderExpression(allocator, stream, tree, start_col, index_expr, new_space);
                    }
                    if (starts_with_comment) {
                        try stream.insertNewline();
                    }
                    return renderToken(tree, stream, rbracket, start_col, space); // ]
                },
                ...
                .UnwrapOptional => {
                    try renderExpression(allocator, stream, tree, start_col, suffix_op.lhs.node, Space.None);
                    try renderToken(tree, stream, tree.prevToken(suffix_op.rtoken), start_col, Space.None); // .
                    return renderToken(tree, stream, suffix_op.rtoken, start_col, space); // ?
                },
                ...
                .Slice => |range| {
                    ...
                    try renderToken(tree, stream, lbracket, start_col, Space.None); // [
                    try renderExpression(allocator, stream, tree, start_col, range.start, after_start_space);
                    try renderToken(tree, stream, dotdot, start_col, after_op_space); // ..
                    if (range.end) |end| {
                        const after_end_space = if (range.sentinel != null) Space.Space else Space.None;
                        try renderExpression(allocator, stream, tree, start_col, end, after_end_space);
                    }
                    if (range.sentinel) |sentinel| {
                        const colon = tree.prevToken(sentinel.firstToken());
                        try renderToken(tree, stream, colon, start_col, Space.None); // :
                        try renderExpression(allocator, stream, tree, start_col, sentinel, Space.None);
                    }
                    return renderToken(tree, stream, suffix_op.rtoken, start_col, space); // ]
                },
                ...
                .StructInitializer => |*field_inits| {
                    ...
                    if (field_inits.len == 0) {
                        switch (suffix_op.lhs) {
                            .dot => |dot| try renderToken(tree, stream, dot, start_col, Space.None),
                            .node => |node| try renderExpression(allocator, stream, tree, start_col, node, Space.None),
                        }
                        try renderToken(tree, stream, lbrace, start_col, Space.None);
                        return renderToken(tree, stream, suffix_op.rtoken, start_col, space);
                    }
                    ...
                    const expr_outputs_one_line = blk: {
                        ...
                        while (it.next()) |field_init| {
                            ...
                            try renderExpression(allocator, &find_stream.stream, tree,  &dummy_col, field_init.*, Space.None);
                    ...
                    if (field_inits.len == 1) blk: {
                        ...
                        try renderToken(tree, stream, lbrace, start_col, Space.Space);
                        try renderExpression(allocator, stream, tree, start_col, &field_init.base, Space.Space);
                        return renderToken(tree, stream, suffix_op.rtoken, start_col, space);
                    }
                    ...
                    if (!src_has_trailing_comma and src_same_line and expr_outputs_one_line) {
                        ...
                        while (it.next()) |field_init| {
                            if (it.peek() != null) {
                                try renderExpression(allocator, stream, tree, start_col, field_init.*, Space.None);

                                const comma = tree.nextToken(field_init.*.lastToken());
                                try renderToken(tree, stream, comma, start_col, Space.Space);
                            } else {
                                try renderExpression(allocator, stream, tree, start_col, field_init.*, Space.Space);
                            }
                        }
...

                    {
                        ...
                        try renderToken(tree, stream, lbrace, start_col, Space.Newline);
                        ...
                        while (it.next()) |field_init| {
                            if (it.peek()) |next_field_init| {
                                ...
                                try renderExpression(allocator, stream, tree, start_col, field_init.*, Space.Comma);
                            }
                        }
                    }
                ...
                .ArrayInitializer => |*exprs| {
                    ...
                    if (exprs.len == 1 and tree.tokens.at(exprs.at(0).*.lastToken() + 1).id == .RBrace) {
                        ...
                        try renderToken(tree, stream, lbrace, start_col, Space.None);
                        try renderExpression(allocator, stream, tree, start_col, expr, Space.None);
                        return renderToken(tree, stream, suffix_op.rtoken, start_col, space);
                    }
                    ...
                    if (maybe_row_size) |row_size| {
                        ...
                        while (it.next()) |expr| : (i += 1) {
                            ...
                            var dummy_col: usize = 0;
                            try renderExpression(allocator, &counting_stream.stream, tree, &dummy_col, expr.*, Space.None);
                            ...
                            while (it.next()) |expr| : (i += 1) {
                                if (it.peek()) |next_expr| {
                                    ...
                                    if (tree.tokens.at(tree.nextToken(comma)).id != .MultilineStringLiteralLine) {
                                        ...
                                        try renderToken(tree, stream, comma, start_col, Space.None); // ,
                                    }

                                    try renderExtraNewline(tree, stream, start_col, next_expr.*);
                                } else {
                                    try renderExpression(allocator, stream, tree, start_col, expr.*, Space.Comma); // ,
                                }
                            }
                        }
                        ...
                        while (it.next()) |expr| {
                            if (it.peek()) |next_expr| {
                                ...
                                try renderExpression(allocator, stream, tree, start_col, expr.*, Space.Space);
                            }
                        }
            ...
            switch (flow_expr.kind) {
                .Break => |maybe_label| {
                    if (maybe_label == null and flow_expr.rhs == null) {
                        return renderToken(tree, stream, flow_expr.ltoken, start_col, space); // break
                    }

                    try renderToken(tree, stream, flow_expr.ltoken, start_col, Space.Space); // break
                    if (maybe_label) |label| {
                        const colon = tree.nextToken(flow_expr.ltoken);
                        try renderToken(tree, stream, colon, start_col, Space.None); // :

                        if (flow_expr.rhs == null) {
                            return renderExpression(allocator, stream, tree, start_col, label, space); // label
                        }
                        try renderExpression(allocator, stream, tree, start_col, label, Space.Space); // label
                    }
                },
                .Continue => |maybe_label| {
                    ...
                    if (maybe_label == null and flow_expr.rhs == null) {
                        return renderToken(tree, stream, flow_expr.ltoken, start_col, space); // continue
                    }

                    try renderToken(tree, stream, flow_expr.ltoken, start_col, Space.Space); // continue
                    if (maybe_label) |label| {
                        const colon = tree.nextToken(flow_expr.ltoken);
                        try renderToken(tree, stream, colon, start_col, Space.None); // :

                        return renderExpression(allocator, stream, tree, start_col, label, space);
                    }
                },
                .Return => {
                    if (flow_expr.rhs == null) {
                        return renderToken(tree, stream, flow_expr.ltoken, start_col, space);
                    }
                    try renderToken(tree, stream, flow_expr.ltoken, start_col, Space.Space);
                },
            }

            return renderExpression(allocator, stream, tree, start_col, flow_expr.rhs.?, space);
        },

        .Payload => {
            const payload = @fieldParentPtr(ast.Node.Payload, "base", base);

            try renderToken(tree, stream, payload.lpipe, start_col, Space.None);
            try renderExpression(allocator, stream, tree, start_col, payload.error_symbol, Space.None);
            return renderToken(tree, stream, payload.rpipe, start_col, space);
        },
        ...
        .PointerPayload => {
            ...
            if (payload.ptr_token) |ptr_token| {
                try renderToken(tree, stream, ptr_token, start_col, Space.None);
            }
            try renderExpression(allocator, stream, tree, start_col, payload.value_symbol, Space.None);
            return renderToken(tree, stream, payload.rpipe, start_col, space);
        },
        ...
        .PointerIndexPayload => {
            ...
            if (payload.index_symbol) |index_symbol| {
                ...
                try renderToken(tree, stream, comma, start_col, Space.Space);
                try renderExpression(allocator, stream, tree, start_col, index_symbol, Space.None);
            }
        ...
        .GroupedExpression => {
            ...
            try renderToken(tree, stream, grouped_expr.lparen, start_col, Space.None);
            try renderExpression(allocator, stream, tree, start_col, grouped_expr.expr, Space.None);
            return renderToken(tree, stream, grouped_expr.rparen, start_col, space);
        },

        .FieldInitializer => {
            const field_init = @fieldParentPtr(ast.Node.FieldInitializer, "base", base);

            try renderToken(tree, stream, field_init.period_token, start_col, Space.None); // .
            try renderToken(tree, stream, field_init.name_token, start_col, Space.Space); // name
            try renderToken(tree, stream, tree.nextToken(field_init.name_token), start_col, Space.Space); // =
            return renderExpression(allocator, stream, tree, start_col, field_init.expr, space);
        },

        .IntegerLiteral => {
            const integer_literal = @fieldParentPtr(ast.Node.IntegerLiteral, "base", base);
            return renderToken(tree, stream, integer_literal.token, start_col, space);
        },
        .FloatLiteral => {
            const float_literal = @fieldParentPtr(ast.Node.FloatLiteral, "base", base);
            return renderToken(tree, stream, float_literal.token, start_col, space);
        },
        .StringLiteral => {
            const string_literal = @fieldParentPtr(ast.Node.StringLiteral, "base", base);
            return renderToken(tree, stream, string_literal.token, start_col, space);
        },
        .CharLiteral => {
            const char_literal = @fieldParentPtr(ast.Node.CharLiteral, "base", base);
            return renderToken(tree, stream, char_literal.token, start_col, space);
        },
        .BoolLiteral => {
            const bool_literal = @fieldParentPtr(ast.Node.CharLiteral, "base", base);
            return renderToken(tree, stream, bool_literal.token, start_col, space);
        },
        .NullLiteral => {
            const null_literal = @fieldParentPtr(ast.Node.NullLiteral, "base", base);
            return renderToken(tree, stream, null_literal.token, start_col, space);
        },
        .Unreachable => {
            const unreachable_node = @fieldParentPtr(ast.Node.Unreachable, "base", base);
            return renderToken(tree, stream, unreachable_node.token, start_col, space);
        },
        .ErrorType => {
            const error_type = @fieldParentPtr(ast.Node.ErrorType, "base", base);
            return renderToken(tree, stream, error_type.token, start_col, space);
        },
        .VarType => {
            const var_type = @fieldParentPtr(ast.Node.VarType, "base", base);
            return renderToken(tree, stream, var_type.token, start_col, space);
        },
        .ContainerDecl => {
            const container_decl = @fieldParentPtr(ast.Node.ContainerDecl, "base", base);

            if (container_decl.layout_token) |layout_token| {
                try renderToken(tree, stream, layout_token, start_col, Space.Space);
            }

            switch (container_decl.init_arg_expr) {
                .None => {
                    try renderToken(tree, stream, container_decl.kind_token, start_col, Space.Space); // union
                },
                .Enum => |enum_tag_type| {
                    try renderToken(tree, stream, container_decl.kind_token, start_col, Space.None); // union

                    const lparen = tree.nextToken(container_decl.kind_token);
                    const enum_token = tree.nextToken(lparen);

                    try renderToken(tree, stream, lparen, start_col, Space.None); // (
                    try renderToken(tree, stream, enum_token, start_col, Space.None); // enum

                    if (enum_tag_type) |expr| {
                        try renderToken(tree, stream, tree.nextToken(enum_token), start_col, Space.None); // (
                        try renderExpression(allocator, stream, tree, start_col, expr, Space.None);

                        const rparen = tree.nextToken(expr.lastToken());
                        try renderToken(tree, stream, rparen, start_col, Space.None); // )
                        try renderToken(tree, stream, tree.nextToken(rparen), start_col, Space.Space); // )
                    } else {
                        try renderToken(tree, stream, tree.nextToken(enum_token), start_col, Space.Space); // )
                    }
                },
                .Type => |type_expr| {
                    try renderToken(tree, stream, container_decl.kind_token, start_col, Space.None); // union
                    ...
                    try renderToken(tree, stream, lparen, start_col, Space.None); // (
                    try renderExpression(allocator, stream, tree, start_col, type_expr, Space.None);
                    try renderToken(tree, stream, rparen, start_col, Space.Space); // )
                },
            }
            ...
            if (container_decl.fields_and_decls.len == 0) {
                {
                    stream.pushIndent();
                    defer stream.popIndent();
                    try renderToken(tree, stream, container_decl.lbrace_token, start_col, Space.None); // {
                }
                return renderToken(tree, stream, container_decl.rbrace_token, start_col, space); // }
            }
            ...
            if (src_has_trailing_comma or !src_has_only_fields) {
                ...
                stream.pushIndent();
                defer stream.popIndent();
                try renderToken(tree, stream, container_decl.lbrace_token, start_col, .Newline); // {
                ...
                while (it.next()) |decl| {
                    ...
                    try renderContainerDecl(allocator, stream, tree, start_col, decl.*, space_after_decl);
                }
            } else {
                // All the declarations on the same line
                try renderToken(tree, stream, container_decl.lbrace_token, start_col, .Space); // {

                var it = container_decl.fields_and_decls.iterator(0);
                while (it.next()) |decl| {
                    try renderContainerDecl(allocator, stream, tree, start_col, decl.*, .Space);
                }
            }
        ...
        .ErrorSetDecl => {
            ...
            if (err_set_decl.decls.len == 0) {
                try renderToken(tree, stream, err_set_decl.error_token, start_col, Space.None);
                try renderToken(tree, stream, lbrace, start_col, Space.None);
                return renderToken(tree, stream, err_set_decl.rbrace_token, start_col, space);
            }
            ...
            if (err_set_decl.decls.len == 1) blk: {
                ...
                try renderToken(tree, stream, err_set_decl.error_token, start_col, Space.None); // error
                try renderToken(tree, stream, lbrace, start_col, Space.None); // {
                try renderExpression(allocator, stream, tree, start_col, node, Space.None);
                return renderToken(tree, stream, err_set_decl.rbrace_token, start_col, space); // }
            }
            ...
            try renderToken(tree, stream, lbrace, start_col, Space.Newline); // {
            {
                ...
                while (it.next()) |node| {
                    if (it.peek()) |next_node| {
                        try renderExpression(allocator, stream, tree, start_col, node.*, Space.None);
                        try renderToken(tree, stream, tree.nextToken(node.*.lastToken()), start_col, Space.Newline); // ,

                        try renderExtraNewline(tree, stream, start_col, next_node.*);
                    } else {
                        try renderExpression(allocator, stream, tree, start_col, node.*, Space.Comma);
                    }
                }
            }
        ...
        .ErrorTag => {
            ...
            try renderDocComments(tree, stream, tag, start_col);
            return renderToken(tree, stream, tag.name_token, start_col, space); // name
        },
        ...
        .MultilineStringLiteral => {
            ...
            while (i < multiline_str_literal.lines.len) : (i += 1) {
                ...
                try renderToken(tree, stream, t, start_col, Space.None);
            }
        },
        .UndefinedLiteral => {
            const undefined_literal = @fieldParentPtr(ast.Node.UndefinedLiteral, "base", base);
            return renderToken(tree, stream, undefined_literal.token, start_col, space);
        },
        ...
        .BuiltinCall => {
            ...
            if (mem.eql(u8, tree.tokenSlicePtr(tree.tokens.at(builtin_call.builtin_token)), "@typeOf")) {
                ...
                try renderToken(tree, stream, builtin_call.builtin_token, start_col, Space.None); // @name
            }
            ...
            if (!src_params_trailing_comma) {
                ...
                while (it.next()) |param_node| {
                    ...
                    if (it.peek() != null) {
                        ...
                        try renderToken(tree, stream, comma_token, start_col, Space.Space); // ,
                    }
                }
            } else {
                // one param per line
            ...
            return renderToken(tree, stream, builtin_call.rparen_token, start_col, space); // )
        },
        ...
        .FnProto => {
            ...
            if (fn_proto.visib_token) |visib_token_index| {
                ...
                try renderToken(tree, stream, visib_token_index, start_col, Space.Space); // pub
            }
            ...
            if (fn_proto.extern_export_inline_token) |extern_export_inline_token| {
                ...
                if (tok.id != .Keyword_extern or fn_proto.body_node == null) {
                    try renderToken(tree, stream, extern_export_inline_token, start_col, Space.Space); // extern/export
                } else {
                    cc_rewrite_str = ".C";
            ...
            if (fn_proto.lib_name) |lib_name| {
                try renderExpression(allocator, stream, tree, start_col, lib_name, Space.Space);
            }
            ...
            if (fn_proto.cc_token) |cc_token| {
                ...
                } else try renderToken(tree, stream, cc_token, start_col, Space.Space); // stdcallcc
            }

            const lparen = if (fn_proto.name_token) |name_token| blk: {
                try renderToken(tree, stream, fn_proto.fn_token, start_col, Space.Space); // fn
                try renderToken(tree, stream, name_token, start_col, Space.None); // name
            ...
            if (!src_params_trailing_comma) {
                ...
                while (it.next()) |param_decl_node| {
                    try renderParamDecl(allocator, stream, tree, start_col, param_decl_node.*, Space.None);
                ...
                var it = fn_proto.params.iterator(0);
                while (it.next()) |param_decl_node| {
                    try renderParamDecl(allocator, stream, tree, start_col, param_decl_node.*, Space.Comma);
                }
            }
            ...
            if (fn_proto.align_expr) |align_expr| {
                ...
                try renderToken(tree, stream, align_kw, start_col, Space.None); // align
                try renderToken(tree, stream, align_lparen, start_col, Space.None); // (
                try renderExpression(allocator, stream, tree, start_col, align_expr, Space.None);
                try renderToken(tree, stream, align_rparen, start_col, Space.Space); // )
            }
            ...
            if (fn_proto.section_expr) |section_expr| {
                ...
                try renderToken(tree, stream, section_kw, start_col, Space.None); // section
                try renderToken(tree, stream, section_lparen, start_col, Space.None); // (
                try renderExpression(allocator, stream, tree, start_col, section_expr, Space.None);
                try renderToken(tree, stream, section_rparen, start_col, Space.Space); // )
            }
            ...
            if (fn_proto.callconv_expr) |callconv_expr| {
                ...
                try renderToken(tree, stream, callconv_kw, start_col, Space.None); // callconv
                try renderToken(tree, stream, callconv_lparen, start_col, Space.None); // (
                try renderExpression(allocator, stream, tree, start_col, callconv_expr, Space.None);
                try renderToken(tree, stream, callconv_rparen, start_col, Space.Space); // )
            ...
            switch (fn_proto.return_type) {
                ast.Node.FnProto.ReturnType.Explicit => |node| {
                    return renderExpression(allocator, stream, tree, start_col, node, space);
                },
                ast.Node.FnProto.ReturnType.InferErrorSet => |node| {
                    try renderToken(tree, stream, tree.prevToken(node.firstToken()), start_col, Space.None); // !
                    return renderExpression(allocator, stream, tree, start_col, node, space);
                },
            }
        },
        ...
        .AnyFrameType => {
            ...
            if (anyframe_type.result) |result| {
                try renderToken(tree, stream, anyframe_type.anyframe_token, start_col, Space.None); // anyframe
                try renderToken(tree, stream, result.arrow_token, start_col, Space.None); // ->
                return renderExpression(allocator, stream, tree, start_col, result.return_type, space);
            } else {
                return renderToken(tree, stream, anyframe_type.anyframe_token, start_col, space); // anyframe
            }
        },
        ...
        .Switch => {
            ...
            try renderToken(tree, stream, switch_node.switch_token, start_col, Space.Space); // switch
            try renderToken(tree, stream, tree.nextToken(switch_node.switch_token), start_col, Space.None); // (

            const rparen = tree.nextToken(switch_node.expr.lastToken());
            const lbrace = tree.nextToken(rparen);

            if (switch_node.cases.len == 0) {
                try renderExpression(allocator, stream, tree, start_col, switch_node.expr, Space.None);
                try renderToken(tree, stream, rparen, start_col, Space.Space); // )
                try renderToken(tree, stream, lbrace, start_col, Space.None); // {
                return renderToken(tree, stream, switch_node.rbrace, start_col, space); // }
            }
        ...
        .SwitchCase => {
            ...
            if (switch_case.items.len == 1 or !src_has_trailing_comma) {
                ...
                while (it.next()) |node| {
                    if (it.peek()) |next_node| {
                        ...
                        try renderExpression(allocator, stream, tree, start_col, node.*, Space.Space);
                    }
                }
            } else {
                var it = switch_case.items.iterator(0);
                while (true) {
                    ...
                    if (it.peek()) |next_node| {
                        ...
                        const comma_token = tree.nextToken(node.*.lastToken());
                        try renderToken(tree, stream, comma_token, start_col, Space.Newline); // ,
                        try renderExtraNewline(tree, stream, start_col, next_node.*);
                    } else {
                        try renderExpression(allocator, stream, tree, start_col, node.*, Space.Comma);
                        break;
            ...
            try renderToken(tree, stream, switch_case.arrow_token, start_col, Space.Space); // =>

            if (switch_case.payload) |payload| {
                try renderExpression(allocator, stream, tree, start_col, payload, Space.Space);
            }

            return renderExpression(allocator, stream, tree, start_col, switch_case.expr, space);
        },
        .SwitchElse => {
            const switch_else = @fieldParentPtr(ast.Node.SwitchElse, "base", base);
            return renderToken(tree, stream, switch_else.token, start_col, space);
        },
        .Else => {
            ...
            try renderToken(tree, stream, else_node.else_token, start_col, after_else_space);

            if (else_node.payload) |payload| {
                const payload_space = if (same_line) Space.Space else Space.Newline;
                try renderExpression(allocator, stream, tree, start_col, payload, payload_space);
            }

            if (same_line) {
                return renderExpression(allocator, stream, tree, start_col, else_node.body, space);
            } else {
                stream.pushIndent();
                defer stream.popIndent();
                return renderExpression(allocator, stream, tree, start_col, else_node.body, space);
        ...
        .While => {
            ...
            if (while_node.label) |label| {
                try renderToken(tree, stream, label, start_col, Space.None); // label
                try renderToken(tree, stream, tree.nextToken(label), start_col, Space.Space); // :
            }

            if (while_node.inline_token) |inline_token| {
                try renderToken(tree, stream, inline_token, start_col, Space.Space); // inline
            }

            try renderToken(tree, stream, while_node.while_token, start_col, Space.Space); // while
            try renderToken(tree, stream, tree.nextToken(while_node.while_token), start_col, Space.None); // (
            try renderExpression(allocator, stream, tree, start_col, while_node.condition, Space.None);
...

            {
                ...
                try renderToken(tree, stream, cond_rparen, start_col, rparen_space); // )
            }
            ...
            if (while_node.continue_expr) |continue_expr| {
                ...
                try renderToken(tree, stream, colon, start_col, Space.Space); // :
                try renderToken(tree, stream, lparen, start_col, Space.None); // (

                try renderExpression(allocator, stream, tree, start_col, continue_expr, Space.None);

                try renderToken(tree, stream, rparen, start_col, block_start_space); // )
            }

            {
                stream.pushIndent();
                defer stream.popIndent();
                try renderExpression(allocator, stream, tree, start_col, while_node.body, after_body_space);
            }

            if (while_node.@"else") |@"else"| {
                return renderExpression(allocator, stream, tree, start_col, &@"else".base, space);
            }
        },
        ...
        .For => {
            ...
            try renderToken(tree, stream, for_node.for_token, start_col, Space.Space); // for
            try renderToken(tree, stream, tree.nextToken(for_node.for_token), start_col, Space.None); // (
            try renderExpression(allocator, stream, tree, start_col, for_node.array_expr, Space.None);
            ...
            try renderExpression(allocator, stream, tree, start_col, for_node.payload, space_after_payload); // |x|
...

            {
                stream.pushIndent();
                defer stream.popIndent();
                try renderExpression(allocator, stream, tree, start_col, for_node.body, space_after_body); // { body }
            }

            if (for_node.@"else") |@"else"| {
                return renderExpression(allocator, stream, tree, start_col, &@"else".base, space); // else
            }
        },
        ...
        .If => {
            ...
            try renderToken(tree, stream, if_node.if_token, start_col, Space.Space); // if
            try renderToken(tree, stream, lparen, start_col, Space.None); // (

            try renderExpression(allocator, stream, tree, start_col, if_node.condition, Space.None); // condition
            ...
            if (body_is_if_block) {
                ...
                try renderToken(tree, stream, rparen, start_col, after_rparen_space); // )

                if (if_node.payload) |payload| {
                    try renderExpression(allocator, stream, tree, start_col, payload, Space.BlockStart); // |x|
                }

                if (if_node.@"else") |@"else"| {
                    try renderExpression(allocator, stream, tree, start_col, if_node.body, Space.SpaceOrOutdent);
                    return renderExpression(allocator, stream, tree, start_col, &@"else".base, space);
                } else {
                    return renderExpression(allocator, stream, tree, start_col, if_node.body, space);
                }
            }
            ...
            if (src_has_newline) {
                ...
                if (if_node.payload) |payload| {
                    try renderExpression(allocator, stream, tree, start_col, payload, Space.Newline);
                }
                ...
                if (if_node.@"else") |@"else"| {
                    ...
                    try renderExpression(allocator, stream, tree, start_col, if_node.body, Space.Newline);

                    if (else_is_block) {
                        try renderToken(tree, stream, @"else".else_token, start_col, Space.Space); // else

                        if (@"else".payload) |payload| {
                            try renderExpression(allocator, stream, tree, start_col, payload, Space.Space);
                        }

                        return renderExpression(allocator, stream, tree, start_col, @"else".body, space);
                    } else {
                        const after_else_space = if (@"else".payload == null) Space.Newline else Space.Space;
                        try renderToken(tree, stream, @"else".else_token, start_col, after_else_space); // else
            ...
            if (if_node.@"else") |@"else"| {
                try renderExpression(allocator, stream, tree, start_col, if_node.body, Space.Space);
                try renderToken(tree, stream, @"else".else_token, start_col, Space.Space);
        ...
        .Asm => {
            ...
            try renderToken(tree, stream, asm_node.asm_token, start_col, Space.Space); // asm

            if (asm_node.volatile_token) |volatile_token| {
                try renderToken(tree, stream, volatile_token, start_col, Space.Space); // volatile
                try renderToken(tree, stream, tree.nextToken(volatile_token), start_col, Space.None); // (
            } else {
                try renderToken(tree, stream, tree.nextToken(asm_node.asm_token), start_col, Space.None); // (
            }

            if (asm_node.outputs.len == 0 and asm_node.inputs.len == 0 and asm_node.clobbers.len == 0) {
                try renderExpression(allocator, stream, tree, start_col, asm_node.template, Space.None);
                return renderToken(tree, stream, asm_node.rparen, start_col, space);
            }

            try renderExpression(allocator, stream, tree, start_col, asm_node.template, Space.Newline);
            ...
            contents: {
                ...
                const colon2 = if (asm_node.outputs.len == 0) blk: {
                    try renderToken(tree, stream, colon1, start_col, Space.Newline); // :
                    break :blk tree.nextToken(colon1);
                } else blk: {
                    try renderToken(tree, stream, colon1, start_col, Space.Space); // :
                    ...
                    while (true) {
                        ...
                        if (it.peek()) |next_asm_output| {
                            ...
                            try renderExpression(allocator, stream, tree, start_col, node, Space.Newline);
                            break :contents;
                        } else {
                            try renderExpression(allocator, stream, tree, start_col, node, Space.Newline);
                ...
                const colon3 = if (asm_node.inputs.len == 0) blk: {
                    try renderToken(tree, stream, colon2, start_col, Space.Newline); // :
                    break :blk tree.nextToken(colon2);
                } else blk: {
                    try renderToken(tree, stream, colon2, start_col, Space.Space); // :
                ...
                try renderToken(tree, stream, colon3, start_col, Space.Space); // :
                ...
                while (true) {
                    ...
                    if (it.peek() == null) {
                        try renderExpression(allocator, stream, tree, start_col, clobber_node, Space.Newline);
                        break :contents;
                    } else {
                        try renderExpression(allocator, stream, tree, start_col, clobber_node, Space.None);
        ...
        .AsmInput => {
            ...
            try stream.write("[");
            try renderExpression(allocator, stream, tree, start_col, asm_input.symbolic_name, Space.None);
            try stream.write("] ");
            try renderExpression(allocator, stream, tree, start_col, asm_input.constraint, Space.None);
            try stream.write(" (");
            try renderExpression(allocator, stream, tree, start_col, asm_input.expr, Space.None);
            return renderToken(tree, stream, asm_input.lastToken(), start_col, space); // )
        },
        ...
        .AsmOutput => {
            ...
            try stream.write("[");
            try renderExpression(allocator, stream, tree, start_col, asm_output.symbolic_name, Space.None);
            try stream.write("] ");
            try renderExpression(allocator, stream, tree, start_col, asm_output.constraint, Space.None);
            try stream.write(" (");

            switch (asm_output.kind) {
                ast.Node.AsmOutput.Kind.Variable => |variable_name| {
                    try renderExpression(allocator, stream, tree, start_col, &variable_name.base, Space.None);
                },
                ast.Node.AsmOutput.Kind.Return => |return_type| {
                    try stream.write("-> ");
                    try renderExpression(allocator, stream, tree, start_col, return_type, Space.None);
                },
            }

            return renderToken(tree, stream, asm_output.lastToken(), start_col, space); // )
        },

        .EnumLiteral => {
            const enum_literal = @fieldParentPtr(ast.Node.EnumLiteral, "base", base);

            try renderToken(tree, stream, enum_literal.dot, start_col, Space.None); // .
            return renderToken(tree, stream, enum_literal.name, start_col, space); // name
        },
...
fn renderVarDecl(
    ...
    if (var_decl.visib_token) |visib_token| {
        try renderToken(tree, stream, visib_token, start_col, Space.Space); // pub
    }

    if (var_decl.extern_export_token) |extern_export_token| {
        try renderToken(tree, stream, extern_export_token, start_col, Space.Space); // extern

        if (var_decl.lib_name) |lib_name| {
            try renderExpression(allocator, stream, tree, start_col, lib_name, Space.Space); // "lib"
        }
    }

    if (var_decl.comptime_token) |comptime_token| {
        try renderToken(tree, stream, comptime_token, start_col, Space.Space); // comptime
    }

    if (var_decl.thread_local_token) |thread_local_token| {
        try renderToken(tree, stream, thread_local_token, start_col, Space.Space); // threadlocal
    }
    try renderToken(tree, stream, var_decl.mut_token, start_col, Space.Space); // var
    ...
    try renderToken(tree, stream, var_decl.name_token, start_col, name_space);

    if (var_decl.type_node) |type_node| {
        try renderToken(tree, stream, tree.nextToken(var_decl.name_token), start_col, Space.Space);
        const s = if (var_decl.align_node != null or
            var_decl.section_node != null or
            var_decl.init_node != null) Space.Space else Space.None;
        try renderExpression(allocator, stream, tree, start_col, type_node, s);
    }
    ...
    if (var_decl.align_node) |align_node| {
        ...
        try renderToken(tree, stream, align_kw, start_col, Space.None); // align
        try renderToken(tree, stream, lparen, start_col, Space.None); // (
        try renderExpression(allocator, stream, tree, start_col, align_node, Space.None);
        const s = if (var_decl.section_node != null or var_decl.init_node != null) Space.Space else Space.None;
        try renderToken(tree, stream, rparen, start_col, s); // )
    }
    ...
    if (var_decl.section_node) |section_node| {
        ...
        try renderToken(tree, stream, section_kw, start_col, Space.None); // linksection
        try renderToken(tree, stream, lparen, start_col, Space.None); // (
        try renderExpression(allocator, stream, tree, start_col, section_node, Space.None);
    ...
    if (var_decl.init_node) |init_node| {
        ...
        try renderToken(tree, stream, var_decl.eq_token.?, start_col, s); // =
        try renderExpression(allocator, stream, tree, start_col, init_node, Space.None);
    }

    try renderToken(tree, stream, var_decl.semicolon_token, start_col, Space.Newline);
}
...
fn renderParamDecl(
    ...
    try renderDocComments(tree, stream, param_decl, start_col);

    if (param_decl.comptime_token) |comptime_token| {
        try renderToken(tree, stream, comptime_token, start_col, Space.Space);
    }
    if (param_decl.noalias_token) |noalias_token| {
        try renderToken(tree, stream, noalias_token, start_col, Space.Space);
    }
    if (param_decl.name_token) |name_token| {
        try renderToken(tree, stream, name_token, start_col, Space.None);
        try renderToken(tree, stream, tree.nextToken(name_token), start_col, Space.Space); // :
    }
    if (param_decl.var_args_token) |var_args_token| {
        try renderToken(tree, stream, var_args_token, start_col, space);
    } else {
        try renderExpression(allocator, stream, tree, start_col, param_decl.type_node, space);
    }
}
    ...
    switch (base.id) {
        .VarDecl => {
            ...
            if (base.requireSemiColon()) {
                try renderExpression(allocator, stream, tree, start_col, base, Space.None);

                const semicolon_index = tree.nextToken(base.lastToken());
                assert(tree.tokens.at(semicolon_index).id == .Semicolon);
                try renderToken(tree, stream, semicolon_index, start_col, Space.Newline);
            } else {
                try renderExpression(allocator, stream, tree, start_col, base, Space.Newline);
            }
        },
    }
}
...
fn renderTokenOffset(
    ...
    if (space == Space.BlockStart) {
        //if (start_col.* < indent + indent_delta)
            //return renderToken(tree, stream, token_index, start_col, Space.Space);
        try renderToken(tree, stream, token_index, start_col, Space.Newline);
        //start_col.* = indent;
        return;
    }
    ...
    if (space == Space.Comma) switch (next_token.id) {
        .Comma => return renderToken(tree, stream, token_index + 1, start_col, Space.Newline),
        .LineComment => {
            try stream.write(", ");
            return renderToken(tree, stream, token_index + 1, start_col, Space.Newline);
        },
        else => {
    ...
    while (true) {
        ...
        try stream.insertNewlines(newline_count);
...
fn renderToken(
    ...
    return renderTokenOffset(tree, stream, token_index, start_col, space, 0);
}
...
fn renderDocComments(
    ...
    while (it.next()) |line_token_index| {
        if (line_token_index.* < first_token) {
            try renderToken(tree, stream, line_token_index.*, start_col, Space.Newline);
        } else {
            try renderToken(tree, stream, line_token_index.*, start_col, Space.NoComment);
            try stream.insertNewline();
        }
    }
}
